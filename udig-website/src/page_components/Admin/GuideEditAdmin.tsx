...
export default function GuideEditAdmin() {
    ...
    const onSubmit = async () => {
        ...
        try {
            const res = await axios.put(
                `${API_BASE}/guides/${id}`,
                { title, description, contents: content, category, readTime },
                { headers: { Authorization: `Bearer ${token}` } }
            );

            if (res.status === 200) navigate(`/get-involved/guides/${id}`);
            if (res.status === 401) alert("Login Expired. Please log in again.");
        } catch (err: any) {
            console.error("Update guide failed:", err.response?.data || err.message);
            alert(err.response?.data || "Failed to update guide");
        }
    };
